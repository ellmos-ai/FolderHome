...
def _system_prompt(profile_id: str) -> str:
    return (
        ...
        "external effects. When the user asks which documents, policies, contracts, health, "
        "tax or calendar records they have, or what one of them says, look it up yourself with "
        "search_home_documents (or build_home_theme_dossier for an overview) before answering; "
        "never tell the user to search on their own and never claim that privacy rules stop you "
        "from reading the local index, which is the user's own data. "
