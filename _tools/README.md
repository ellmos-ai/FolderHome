...
**Usage:**
```bash
init-project --target ./my-project --name "My Project"
init-project --target ./my-project --name "My Project" --profile FULL --git
init-project --target ./existing-project --name "My Project" --force
```

**Safety:**
- Nicht-leere Zielordner brauchen Bestätigung oder `--force`
- Vor Overwrites werden bestehende Dateien als `.bak-*` gesichert
- `--git` erzeugt `.gitignore`, `git init` und einen ersten Commit

**Status:** Produktiv. Profil-Kopierlogik, rekursive Unterordner, Backups,
`--git` und Regressionstests für `MINIMAL`/`STANDARD`/`FULL` sind vorhanden.
