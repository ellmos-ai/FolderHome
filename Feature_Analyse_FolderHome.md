...
| Amazon Bedrock | Handoff | The same agent supports `BedrockModel`, but only with model ID, region and separate network/data transfer gates; verified live on 2026-09-12 through the local app and on 2026-09-13 end to end in the deployed AgentCore demo (EU Nova Micro inference profile, two bounded model turns, `manage.py verify` status `verified`); the public site stays unpublished until the owner enables it |
...
| Hosted models (Anthropic, OpenAI) | Locally implemented | `AnthropicModel` and `OpenAIModel` on the same agent loop; both leave the machine and therefore need `--allow-network` and `--approve-sensitive-cloud-data`. The key is never a setting: it is read from `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` and appears in no plan, status or log. Not live tested |
| Model presets | Locally implemented | Named provider settings in `launch.json`; switching models is activating a preset and saving. A command-line flag beats a flat field, a flat field beats the active preset. Inactive presets are validated too |
| Setup: dialogs, sources, calendar | Locally implemented | A native folder dialog runs in a child process, a source purpose takes several folders, and `calendar.json` plus `calendar-accounts.json` are written through the same hash-bound confirm-verify-replace path. Honest limit: only the calendar commands read those two files, `app serve` does not |
| Subscriptions section | Locally implemented | The installer shows the ready-made Claude Code command and Codex TOML block from the same `integration_plan` the CLI prints. With a subscription agent the agent is the brain, so FolderHome needs no key of its own; no subscription is read, stored or checked |
| Profile management | Locally implemented | The installer adds, renames, edits and deletes profiles and owns its own profile folder; a deletion cascades in the same plan onto folder bindings and calendar accounts, at least one profile remains, and the file moves into a dated `.deleted-` folder instead of being removed. The shipped examples stay a template and are refused as a write target |
| Agent-readable guide | Locally implemented | `llms.txt` at the repository root and under `site/` names the MCP and HTTP entry points, the safety model and the providers; a test asserts every route, schema and tool it names exists in the service |
